        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="icon_type" id="icon_type" class="form-control" onchange="toggleIconFields()">
                    <option value="emoji" {{ old('icon_type', $service->icon_type) == 'emoji' ? 'selected' : '' }}>Emoji / Teks</option>
                    <option value="feather" {{ old('icon_type', $service->icon_type) == 'feather' ? 'selected' : '' }}>Feather Icon</option>
                    <option value="image" {{ old('icon_type', $service->icon_type) == 'image' ? 'selected' : '' }}>Upload Gambar / SVG</option>
                </select>
            ...
            <div class="form-group" id="field_icon_text">
                ...
                <input type="text" name="icon_text" class="form-control" value="{{ old('icon_text', $service->icon_type !== 'image' ? $service->icon : '') }}">
            ...
            <div class="form-group" id="field_icon_image" style="display: none;">
                ...
                @if($service->icon_type === 'image' && $service->icon)
                    <div style="margin-top: 10px;">
                        <img src="{{ Storage::url($service->icon) }}" alt="Current Icon" style="max-height: 50px;">
                    </div>
                @endif
        ...
        <div class="form-row">
            ...
            <div class="form-group" style="display: flex; align-items: flex-end; padding-bottom: 4px;">
                <div class="form-check">
                    ...
                    <label for="is_active" style="color: var(--text-light); font-size: 0.9rem;">Aktif</label>
                </div>
            </div>
        </div>
        
        <button type="submit" class="btn btn-primary"><i data-feather="save" style="width: 16px; height: 16px;"></i> Perbarui Layanan</button>
    </form>
</div>
